    ...
    $.fn.treeview = function (settings) {
        var dfop =
            {
                ...
                datatype: "json",
                url: false,
                cbiconpath: $.tms.url.root() + "/content/images/icons/",
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            if (deep == 1) {
                ...
                ht.push("<img class='bbit-tree-icon' src='" + dfop.cbiconpath + "s.gif'/>");
                var parent = nd.parent.parent;
                var isLast = parent.ChildNodes[parent.ChildNodes.length - 1] == nd.parent;
                for (var j = 1; j < deep; j++) {
                    if (!isLast) {
                        ht.push("<img class='bbit-tree-elbow-line' src='" + dfop.cbiconpath + "s.gif'/>");
                    } else {
                        ht.push("<img class='bbit-tree-elbow-end-line' src='" + dfop.cbiconpath + "s.gif'/>");
                    }
                }
            }
            ht.push("</span>");
            ...
            if (nd.hasChildren) {
                if (nd.isexpand) {
                    if (deep == 0) {
                        cs.push(isend ? "bbit-tree-elbow-root-minus" : "bbit-tree-elbow-minus");
                    } else {
                    ...
                    if (deep == 0) {
                        cs.push(isend ? "bbit-tree-elbow-root-plus" : "bbit-tree-elbow-plus");
        ...
        function nodeclick(e) {
            ...
            if (et.tagName == "IMG") {
                // +号需要展开
                if ($(et).hasClass("bbit-tree-elbow-plus") || $(et).hasClass("bbit-tree-elbow-end-plus") || $(et).hasClass("bbit-tree-elbow-root-plus")) {
                    ...
                    if ($(et).hasClass("bbit-tree-elbow-plus")) {
                        ...
                        $(et).swapClass("bbit-tree-elbow-root-plus", "bbit-tree-elbow-root-minus");
                    }
                    $(this).swapClass("bbit-tree-node-collapsed", "bbit-tree-node-expanded");
                }
                else if ($(et).hasClass("bbit-tree-elbow-minus") || $(et).hasClass("bbit-tree-elbow-end-minus") || $(et).hasClass("bbit-tree-elbow-root-minus")) {  //- 号需要收缩                    
                    $(this).next().hide();
                    if ($(et).hasClass("bbit-tree-elbow-minus")) {
                        ...
                        $(et).swapClass("bbit-tree-elbow-root-minus", "bbit-tree-elbow-root-plus");
                    }
